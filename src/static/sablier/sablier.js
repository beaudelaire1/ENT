document.addEventListener("DOMContentLoaded", () => {
  ...
  function drawWave(progress){
    resize();const w=canvas.clientWidth,h=canvas.clientHeight,cx=w/2,cy=h*.46,r=Math.min(w,h)*.33,
  ...
  // Bougie : la cire descend, la flamme vacille et s'éteint à la fin.
  function drawCandle(progress){
    resize();const w=canvas.clientWidth,h=canvas.clientHeight,cx=w/2,
      accent=getComputedStyle(app).getPropertyValue("--focus-accent").trim(),
      border=getComputedStyle(app).getPropertyValue("--focus-border").trim(),
      bodyW=Math.min(w,h)*.17,full=h*.52,base=h*.78,top=base-full*progress;
    ctx.clearRect(0,0,w,h);
    ...
    if(progress>.001){
      ...
      ctx.beginPath();ctx.roundRect(cx-bodyW/2,top,bodyW,base-top,bodyW*.18);ctx.fill();ctx.globalAlpha=1;
  ...
  function drawBeads(progress){
    resize();const w=canvas.clientWidth,h=canvas.clientHeight,cols=6,rows=6,total=cols*rows,
      accent=getComputedStyle(app).getPropertyValue("--focus-accent").trim(),
      border=getComputedStyle(app).getPropertyValue("--focus-border").trim(),
      span=Math.min(w,h)*.62,step=span/(cols-1),ox=w/2-span/2,oy=h*.46-span/2,r=Math.max(5,step*.19),
  ...
  function drawMoon(progress){
    resize();const w=canvas.clientWidth,h=canvas.clientHeight,cx=w/2,cy=h*.46,r=Math.min(w,h)*.3,
      accent=getComputedStyle(app).getPropertyValue("--focus-accent").trim(),
      border=getComputedStyle(app).getPropertyValue("--focus-border").trim();
    ctx.clearRect(0,0,w,h);
  ...
  const prefForm=$("#focus-preferences"),durationHidden=prefForm.querySelector("#id_default_duration_seconds");prefForm.addEventListener("submit",()=>{durationHidden.value=Math.round(state.total);prefForm.querySelector("#id_session_intention").value=state.intention;prefForm.querySelector("#id_mode").value=state.mode;prefForm.querySelector("#id_ambience").value=state.ambience;prefForm.querySelector("#id_focus_level").value=state.focusLevel;prefForm.querySelector("#id_warning_seconds").value=state.warning;});
